...
public class ItemController {
    ...
    public void deleteItem(@PathVariable String appId, @PathVariable String env,
        ...
        NamespaceDTO namespace = namespaceService.loadNamespaceBaseInfo(appId, Env.fromString(env), clusterName,
                namespaceName);
    ...
    @GetMapping("/apps/{appId}/envs/{env}/clusters/{clusterName}/namespaces/{namespaceName}/branches/{branchName" +
            "}/items")
    ...
    public List<ItemDiffs> diff(@RequestBody NamespaceSyncModel model) {
        ...
        for (ItemDiffs diff : itemDiffs) {
            ...
            if (permissionValidator
                ...
                diff.setExtInfo("You are not this project's administrator, nor you have edit or release permission " +
                        "for the namespace in environment: " + namespace.getEnv());
            }
        }
    ...
    public ResponseEntity<Void> update(@PathVariable String appId, @PathVariable String namespaceName,
        ...
        if (!hasPermission) {
            ...
            for (NamespaceIdentifier namespaceIdentifier : model.getSyncToNamespaces()) {
                ...
                hasPermission &= permissionValidator.hasModifyNamespacePermission(namespaceIdentifier.getAppId(),
        ...
        throw new AccessDeniedException(String.format("You don't have the permission to modify environment: %s",
                envNoPermission));
    }

    @PreAuthorize(value = "@permissionValidator.hasModifyNamespacePermission(#appId, #namespaceName, #env)")
    @PostMapping(value = "/apps/{appId}/envs/{env}/clusters/{clusterName}/namespaces/{namespaceName}/syntax-check",
            consumes = {
                    "application/json"})
