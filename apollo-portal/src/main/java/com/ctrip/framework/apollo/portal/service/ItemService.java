...
public class ItemService {
    ...
    public void updateConfigItemByText(NamespaceTextModel model) {
        ...
        // 解析出改变项
        ItemChangeSets changeSets = resolver.resolve(
                namespaceId,
                configText,
                // 查找历史项
        ...
        Tracer.logEvent(TracerEventType.MODIFY_NAMESPACE, String.format("%s+%s+%s+%s", appId, env, clusterName,
                namespaceName));
    }
    ...
    public void syncItems(List<NamespaceIdentifier> comparedNamespaces, List<ItemDTO> sourceItems) {
        ...
        for (ItemDiffs itemDiff : itemDiffs) {
            ...
            Tracer.logEvent(TracerEventType.SYNC_NAMESPACE, String.format("%s+%s+%s+%s", appId, env, clusterName,
                    namespaceName));
        }
    }
    ...
    public void revokeItem(String appId, Env env, String clusterName, String namespaceName) {
        ...
        releaseItemDTOs.forEach((key, value) -> {
            ...
            if (oldItem == null) {
                ...
                changeSets.addCreateItem(buildNormalItem(0L, namespaceId, key, value, deletedItemDto.getComment(),
                        lineNum.get()));
